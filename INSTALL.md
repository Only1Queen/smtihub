...
The build takes a few minutes the first time. Wait for the database to be
ready:
...
Lots of `OK` lines. This also creates the first appraisal year for you.
...
it before anyone signs in — the audit trail is not protected until this says
`t`.
...
You want `db` and `web` both showing `healthy`, and `proxy` showing `Up`. Give
it about thirty seconds after starting.
...
runs, and the `verify_grants.sql` line at the end is how you find out — it must
still print `t`.
